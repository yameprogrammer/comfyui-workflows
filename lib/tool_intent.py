...
INTENT_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "summary": "문장으로 전체 이미지 편집 (Qwen-Image 2.1 Low VRAM GGUF 2K / 2511)",
        "when": "배경만 밤으로 등 영역 마스크 없이 자연어 문장 지시어 편집, 멀티 레퍼런스 이미지 합성, 네이티브 2K 지원",
        "when_not": "마스크 국소 부위만 수정",
        "keywords": [
            "edit", "instruction", "배경", "편집", "바꿔", "qwen edit", "문장 편집",
            "qwen", "qwen-image", "qwen2.1", "qwen 2.1", "qwen_image_21_edit", "지시어 편집", "자연어 편집",
        ],
        "examples": [
            'python scripts/generate_qwen_edit.py --preset qwen_image_21_edit -i img.png -p "make the background night city, keep the person" -o out.png',
            'python scripts/generate_qwen_edit.py --preset qwen_image_21_edit -i char1.png -i2 char2.png -p "the 2 women are taking a selfie at the park" -o selfie.png',
    ...
    },
    {
        ...
        "alternatives": [
            ...
            {"if": "카메라 경로를 3D로 잠근 뒤 H3가 따라가게", "use": "camera_previz", "cli": "python scripts/generate_previz.py --preset corridor_follow -o plate.mp4"},
        ],
    },
    {
        "id": "camera_previz",
        ...
        "alternatives": [
            {"if": "의도 프리셋만 (3D 없음)", "use": "camera_move", "cli": "python scripts/generate_camera_move.py -i key.png --preset push_in -o clip.mp4"},
            {"if": "고정 토킹헤드", "use": "minimax_h3 i2v", "cli": "python scripts/generate_minimax_h3.py --task i2v -i face.png -p \"locked tripod, no zoom\" -o talk.mp4"},
            {"if": "Blender 꺼짐", "use": "camera_move", "cli": "python scripts/generate_previz.py --probe"},
        ],
    },
    {
        "id": "idle_loop",
    ...
    },
    {
        ...
        "examples": [
            'python scripts/generate_midi_arrangement.py --chords "Am,F,C,G" --genre acoustic_ballad --density medium -o "%AGENT_WORKSPACE%/beds/bed.mid"',
    ...
    },
    {
        ...
        "alternatives": [
            ...
            {"if": "카메라 경로를 3D 플레이트로 잠금", "use": "camera_previz", "cli": "python scripts/generate_previz.py --preset corridor_follow -o plate.mp4"},
        ],
    },
    {
        "id": "ltx_lora_status",
    ...
    },
    {
        "id": "trellis2_mesh",
        "shelf": "MESH",
        "cli": "python scripts/generate_trellis_mesh.py",
        "script": "generate_trellis_mesh.py",
        "summary": "2D 이미지 → 3D 메쉬 GLB (TRELLIS 2, MIT). 에이전트 MESH 기본",
        "when": "프론트 스틸에서 프랍/메카/그레이박스 메쉬 (기본 work = 1024_cascade PBR)",
        "when_not": "히어로 캐릭터 리그 · 쿼드 토폴로지 · Hunyuan 명시 요청(KR 라이선스 패스) · 영상 I2V",
        "keywords": [
            "3d", "mesh", "glb", "trellis", "trellis2", "메쉬", "3d 모델",
            "image to 3d", "이미지 3d", "generate_trellis_mesh", "오브젝트 메쉬",
            "pbr", "프랍",
        ],
        "examples": [
    ...
    },
    {
        ...
        "summary": "Hunyuan3D / Hy3D GLB. KR Community License 제외 — 에이전트 기본 아님",
        "when": "사용자가 Hunyuan/Hy3D를 이름으로 지정했을 때만",
        "when_not": "한국 상업 로컬 본선 · 일반 이미지→3D (그건 generate_trellis_mesh)",
        "keywords": [
            "hunyuan", "hunyuan3d", "hy3d", "generate_hy3d_mesh",
        ],
        "examples": [
        ...
        "alternatives": [
            {"if": "한국 상업 / 기본 3D", "use": "trellis2_mesh", "cli": "python scripts/generate_trellis_mesh.py -i front.png -o out.glb"},
        ],
    },
    {
        "id": "process_mesh_glb",
        ...
        "when": "TRELLIS/기타 raw GLB 정리·프로토타입 아마추어 부착",
        ...
        "alternatives": [
            {"if": "메쉬 생성부터", "use": "trellis2_mesh", "cli": "python scripts/generate_trellis_mesh.py -i front.png -o out.glb"},
    ...
    },
    {
        ...
        "alternatives": [
            {"if": "메쉬 없음", "use": "trellis2_mesh", "cli": "python scripts/generate_trellis_mesh.py -i front.png -o out.glb"},
...
_PHRASE_BOOSTS: list[tuple[tuple[str, ...], str, float]] = [
    ...
    (("프리비즈",), "camera_previz", 8.5),
    (("previz",), "camera_previz", 8.5),
    (("previs",), "camera_previz", 7.5),
    (("플레이블라스트",), "camera_previz", 8.0),
    (("playblast",), "camera_previz", 8.0),
    (("블렌더", "카메라"), "camera_previz", 7.0),
    (("blender", "camera"), "camera_previz", 6.5),
    (("카메라", "경로"), "camera_previz", 6.5),
    (("카메라", "잠금"), "camera_previz", 6.0),
    (("from-scene",), "camera_previz", 8.0),
    (("커스텀", "장면"), "camera_previz", 7.5),
    (("급카메라",), "camera_previz", 7.0),
    ...
    (("3d",), "trellis2_mesh", 6.0),
    (("메쉬",), "trellis2_mesh", 6.0),
    (("glb",), "trellis2_mesh", 5.0),
    (("trellis",), "trellis2_mesh", 8.0),
    (("trellis2",), "trellis2_mesh", 8.0),
    (("hunyuan",), "hy3d_mesh", 7.0),
    (("hunyuan3d",), "hy3d_mesh", 8.0),
    (("hy3d",), "hy3d_mesh", 7.0),
    (("이미지", "3d"), "trellis2_mesh", 7.0),
    (("image", "to", "3d"), "trellis2_mesh", 7.0),
