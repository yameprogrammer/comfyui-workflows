...
INTENT_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "summary": "18+ 해부학 영역 디테일러 (Impact SEGS + Krea2)",
        "when": "성인 스틸 특정 부위 디테일 보정 (penis/vagina bbox) — **반드시 --i-am-18**",
        "when_not": "미성년·SFW · breast는 segm 미배선 · 일반 얼굴/손 → face/hand detail",
        "keywords": [
            "anatomy detailer", "nsfw detailer", "generate_krea2_anatomy_detail",
            "해부학", "디테일러 18", "adult detail",
        ],
        "examples": [
            "python scripts/generate_krea2_anatomy_detail.py -i nsfw.png -o out.png --region penis --i-am-18",
            "python scripts/generate_krea2_anatomy_detail.py -i nsfw.png -o out.png --region vagina --i-am-18 --denoise 0.28",
        ],
        "alternatives": [
    ...
    },
    {
        ...
        "summary": "MiniMax H3 T2V/I2V/R2V + 네이티브 스테레오 오디오",
        "when": "시댄스급 품질 쇼츠·텍스트→영상+음향·멀티 레퍼 정체성; 히어로 컷",
        "when_not": "에피 본선 I2V 대량(LTX) · 립싱크 CU(s2v) · 초고속 초안만",
        "keywords": [
            "minimax", "minimax h3", "h3", "hailuo", "seedance", "시댄스", "t2v",
            "r2v", "native audio", "stereo audio", "미니맥스", "영상 오디오",
            "open weight video", "fl2va", "ref2va", "minimax_h3",
        ],
        "examples": [
            ...
            'python scripts/generate_minimax_h3.py --task r2v --ref-image hero.png --ref-image style.png -p "Picture 1 identity; Picture 2 style; she walks" -o r2v.mp4',
        ...
        "alternatives": [
            ...
            {"if": "대사 립싱크", "use": "s2v", "cli": "python scripts/generate_s2v.py -i face.png -a line.wav -o talk.mp4"},
...
_PHRASE_BOOSTS: list[tuple[tuple[str, ...], str, float]] = [
    ...
    (("nsfw", "detailer"), "krea2_anatomy_detail", 6.0),
