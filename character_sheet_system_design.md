...
→ P1 (seed/meta CLI) → P2 (create/expand/approve) → 파일럿 mina_park_v1
L3 LoRA / ControlNet / I2V 본구현은 본 트랙 완료 후
```
...
# 5) (이후) I2V
python generate_i2v.py -i shots/ep01_s03.png -p "slow push-in, wind on hair" -o clips/ep01_s03.mp4
```
...
| **P3** | 시트 템플릿·그리드 | pose templates, grid composer | P2 |
| **P4** | ControlNet 시트 WF | 구조 고정 turnaround | P3, 모델 확보 |
| **P5** | LoRA 학습 파이프 | dataset recipe + train + validate (L3) | P2 |
| **P6** | LoRA 연동 생성 WF | T2I/I2I LoRA 주입 | P5 |
| **P7** | Shot-with-character | 스토리 키프레임 CLI | P2/P6 |
...
```text
1순위  P0 + P1 + P2   → 에이전트가 쓸 수 있는 L2 캐릭터 팩
2순위  P7             → 캐릭터로 샷 키프레임
3순위  P5 + P6        → LoRA로 주연급 일관성
4순위  P4             → 시트 규격 미학/정확도
5순위  P8             → I2V와 E2E (I2V 도구 준비와 병렬)
```

> 영상 로드맵의 I2V(P0)와 **병렬**로 캐릭터 L2를 가도 된다.  
> 다만 **스토리 주연 영상**을 목표로 하면 캐릭터 L2/L3 없이 I2V만 키우면 얼굴이 무너진 예쁜 클립만 쌓인다.
...
1. ~~P0 템플릿/스펙~~ ✅
2. **P1 코딩**: [character_impl_spec.md](character_impl_spec.md) Ticket P1-A → P1-B
3. **P2 코딩**: create / expand / approve
4. **파일럿** `mina_park_v1` E2E (ComfyUI 가동 필요)
5. 성공 시 P7 `shot_with_character` → I2V 트랙과 합류
