...
| `work_*` | format의 `default_work_preset` | **`work_*_720` 기본** (2026-07-18); `work_*_540` = draft/fast |
...
|-----------|--------|--------|-------|
| **`work_16x9_720`** | **1280×720** | 16:9 | **work 기본 (2026-07-18)** |
| `work_16x9_540` | 960×544 | 16:9 | work draft/fast |
| `work_9x16_720` | 720×1280 | 9:16 | work 기본 (shorts) |
| `work_9x16_540` | 544×960 | 9:16 | work draft |
| `work_4x3_720` / `work_3x4_720` / `work_1x1_720` | 720p-class | … | work 기본 |
| deliver tiers | short-edge 1080/2160 | — | `upscale_backends.json` |
...
| Upscale | `scripts/upscale_*` — SeedVR2 / RTX VSR / ESRGAN (최대 4K) | work → 1080/1440/2160 |
...
```json
{
  ...
  "default_work_preset": "work_16x9_720",
  "default_deliver_tier": "deliver_1080",
  "presets": {
    "work_16x9_720": { "width": 1280, "height": 720, "aspect": "16:9", "stage": "work" },
    "work_16x9_540": { "width": 960, "height": 544, "aspect": "16:9", "stage": "work" },
    "work_9x16_720": { "width": 720, "height": 1280, "aspect": "9:16", "stage": "work" }
  },
  "backends": {
    "ltx23_aio_i2v": {
      "workflow": "ltx23AllInOneWorkflowForRTX_v44",
      "runner": "generate_i2v.py / generate_s2v.py",
      "strengths": ["agent_default_i2v", "motion", "film"],
      "notes": "DEFAULT I2V; work=720p"
    },
    "wan22": {
      "workflow": "I2V-wan22-a14b.json",
      "runner": "generate_i2v.py",
      "strengths": ["character_i2v", "fallback"],
      "notes": "GGUF dual high/low + lightx2v; quality fallback"
    },
    "ltx23": {
      "workflow": "alias → ltx23_aio_i2v",
      "runner": "generate_i2v.py",
...
| D4 | `upscale_image/video.py` multi-backend (≤4K, format-aware) | ✅ |
...
| 프리셋 | ✅ 기본 **`work_16x9_720` (1280×720)**; draft `work_*_540`; 스모크 `work_1x1_smoke` |
...
| 2026-07-18 | **기본 work → 720p** (`work_*_720`); LTX work longer_edge 1280 |
